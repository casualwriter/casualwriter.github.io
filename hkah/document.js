;(function(){ 
  ...
  md.parser = function( mdstr ) {
    ...
    mdstr = mdstr.replace(/\s(https?\:\/\/.*?)\s/gm, '<a href="$1">$1</a>')
...
//=============================================================================
// 20220915, ck, for hkah documentation
//=============================================================================
window.onload = function () {
  
  var content = document.getElementsByTagName('xmp')[0] || document.body
  
  var html = '<div class="container"><header id=heading>' + (content.title||document.title) + '</header>'
  
  html += '\n<div id=toc></div>\n<div id=content>' + md.html(md.raw=content.innerHTML) + '</div></div>'; 
  
  // add shortcut for edit current page.
  var thisFile = window.location.pathname + (window.location.pathname.substr(-1)=='/'?'index.html':'')
  
  html += '<a href="hkah://edit/' + thisFile.replace('/doc/','q:/documents/')  
  html += '" accesskey=s style="display:none">Edit</a>';
  ...
  if (content.className == 'toc') md.toc( 'content', 'toc', { button:'X', scrollspy:'active'} )
...
function tocToggle(show) {
  var disp = document.getElementById('toc').style.display
  document.getElementById('toc').style.display = show||(disp=='none')? 'block' : 'none'
}
