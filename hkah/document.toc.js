...
;(function(){ 
 ...
 function Lexer(options) {
  ...
  Lexer.prototype.token = function(src,top) { 
	...
	while(src) { 
		...
		if (top&&(cap=this.rules.nptable.exec(src))) {
		  src=src.substring(cap[0].length);
		  item={ type:'table',header:cap[1].replace(/^ *| *\| *$/g,'').split(/ *\| */),
  ...
  var inline={	escape:/^\\([\\`*{}\[\]()#+\-.!_>])/,
		...
		inline.gfm=merge( {}, inline.normal,{ escape:replace(inline.escape)('])','~|])')()
											  ,url:/^(https?:\/\/[^\s<]+[^<.,:;"')\]\s])/
											  ,del:/^~~(?=\S)([\s\S]*?\S)~~/
			  ...
			  if( cap=this.rules.url.exec(src) ) { 
			   ...
			   if ( cap=this.rules.em.exec(src) ) { 
				...
				if(cap=this.rules.br.exec(src)) { 
				  src=src.substring(cap[0].length);out+='<br>';
				  continue;
				}
				...
				if (cap=this.rules.text.exec(src)) { 
				  src=src.substring(cap[0].length);out+=escape(cap[0]);
				  continue;
				}
				if(src) { 
	  ...
	  switch  (this.token.type)
	   {case'space':{return'';}
		...
		case'code':{if(this.options.highlight){ 
						...
						if(code!=null&&code!==this.token.text) {this.token.escaped=true;this.token.text=code;}
					}
		...
		case'table':{var body='',heading,i,row,cell,j;
						body+='<thead>\n<tr>\n';
		...
		case'blockquote_start':{var body='';
		...
		case'list_item_start':{var body='';
								while(this.next().type!=='list_item_end'){
									body+=this.token.type==='text'?this.parseText():this.tok();
								}
								return'<li>'+body+'</li>\n';
							}
		case'loose_item_start':{var body='';
	...
	marked.options=marked.setOptions=function(opt) { 
		...
		return marked;};
		marked.defaults = { gfm:true,tables:true,breaks:false, pedantic:false,sanitize:false
							,smartLists:false, silent:false,highlight:null,langPrefix:'lang-'
						  };
...
console.log( scriptEls[i].src )
if (scriptEls[i].src.match('document.js')) {
...
linkEl.href = originBase + '/document.css';
...
newNode.innerHTML = '<header id=heading>'+document.title+'</header><div id=toc></div><div id=content></div>'
...
//===========extent code for hkah ==================
// ck. 202012, add shortcut for edit current page.
var thisFile = window.location.pathname + (window.location.pathname.substr(-1)=='/'?'index.html':'')
infoEl = document.createElement('a'); 
infoEl.setAttribute('href','hkah://edit/'+thisFile.replace('/doc/','q:/documents/') );
infoEl.accessKey = 's'
infoEl.style.cssText = 'display:none'
document.body.appendChild(infoEl)
  
// ck. 202006, add info box
var infoEl = document.createElement('div');
infoEl.id ='infoTitle'
infoEl.style.cssText = "display:none;position:fixed;top:62px;left:45%;width:52%;height:28px;padding:4px;background:#888;color:white;border:1px solid"
infoEl.onclick = hideInfo
document.body.appendChild(infoEl)
	
var infoEl = document.createElement('iframe');
infoEl.id ='infoFrame'
infoEl.style.cssText = "display:none;position:fixed;top:90px;left:45%;width:52%;height:85%;padding:4px;background:#eee;border:1px solid"
document.body.appendChild(infoEl)

// ck. 202007, add infoToc
//var infoEl = document.createElement('div');
//infoEl.id ='infoToc'
//infoEl.style.cssText = "position:fixed;top:60px;left:80%;width:19%;padding:4px;background:#eee;border:1px solid grey"
//document.body.appendChild()
var toc = document.querySelectorAll('h2,h3,h4')
var html = '<button style="float:right" onclick="document.getElementById(\'toc\').style.display=\'none\'">X</button>'
html += '<ul><h3>Table of Contents</h3>';
...
document.getElementById('toc').innerHTML = html + "</ul>";

// hkah functions
function showInfo(url,title,width) { 
	document.getElementById('infoTitle').style.display='block'
	document.getElementById('infoTitle').innerText = title || 'Information'
...
function showSQL(sql, title, width) {
	document.getElementById('infoTitle').style.display='block'
	document.getElementById('infoTitle').innerText = title || 'SQL'
	document.getElementById('infoFrame').style.display = 'block'
	document.getElementById('infoFrame').src = sql
}	
